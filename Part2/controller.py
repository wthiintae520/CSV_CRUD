...
class Controller:
    ...
    def reload(self):
        ...
        try:
            ...
            for i in range(101):
                ...
                elements = line.split(",")
                data = model.Data(elements[0][1:-1], elements[1][1:-1], elements[2][1:-1], elements[3][1:-1], elements[4], elements[5], elements[6][0:3])
    ...
    def display(self):
        ...
        for i in range(int(start), int(end) + 1):
            ...
            print("{:^13} {:^13} {:^13} {:^13} {:^7} {:^7} {:^17}".format(data_list[i].get_source(), data_list[i].get_latin_name(), data_list[i].get_english_name(), data_list[i].get_french_name(), data_list[i].get_year(), data_list[i].get_month(), data_list[i].get_number_otoliths()))
    ...
    def create(self):
        ...
        new_data = model.Data(new_source, new_latin_name, new_english_name, new_french_name, new_year, new_month, new_number_otoliths)
...
# reading 100 data from the dataset, the first line is column, so read 101 times
for i in range(101):
    line = csvfile.readline()
    elements = line.split(",")
    data = model.Data(elements[0][1:-1], elements[1][1:-1], elements[2][1:-1], elements[3][1:-1], elements[4], elements[5], elements[6][0:-1])
    data_list.append(data)
